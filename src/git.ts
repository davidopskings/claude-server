...
import type { AgentJob, CodeRepository } from "./db/index.js";
import { getRepositoryById, listRepositories } from "./db/index.js";
...
export async function ensureBareRepo(repo: CodeRepository): Promise<string> {
	...
	if (!existsSync(barePath)) {
		console.log(
			`Cloning bare repository for ${repo.owner_name}/${repo.repo_name}...`,
		);
		execSync(
...
export async function fetchAllRepos(): Promise<
	{ repo: string; success: boolean; error?: string }[]
> {
...
export async function createWorktree(
	...
	if (branchExists) {
			...
			for (const line of lines) {
				if (line.startsWith("worktree ")) {
					...
					if (branchName === job.branch_name) {
						...
						console.log(
							`Branch ${job.branch_name} is checked out at ${currentWorktreePath}, removing...`,
						);
						try {
		...
		execSync(`git worktree add "${worktreePath}" ${job.branch_name}`, {
			cwd: barePath,
			stdio: "pipe",
		});
		...
		try {
			...
			console.log(
				`Warning: Could not fast-forward ${job.branch_name}, using local state`,
			);
		}
	} else {
		...
		const remoteSha = execSync(`git ls-remote origin ${defaultBranch}`, {
			cwd: barePath,
			encoding: "utf-8",
		})
			.split("\t")[0]
...
export async function removeWorktree(
	...
	try {
		...
		console.error(`Failed to remove worktree: ${(err as Error).message}`);
		// Try force cleanup
		try {
			if (existsSync(worktreePath)) {
				rmSync(worktreePath, { recursive: true, force: true });
...
export async function createPullRequest(
	...
	try {
		...
		prNumber = parseInt(prUrl.split("/").pop() || "0", 10);
	...
	try {
		...
		filesChanged = parseInt(filesOutput.trim(), 10) || 0;
	} catch {
...
export async function cloneRepo(
	repoId: string,
): Promise<{ repo: string; success: boolean; path?: string; error?: string }> {
...
export async function cloneAllRepos(): Promise<
	{ repo: string; success: boolean; path?: string; error?: string }[]
> {
